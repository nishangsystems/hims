...
class CustomApplicationController extends Controller
{
    ...
    public function index(){
        ...
        $data['applications'] = ApplicationForm::where(['transaction_id'=>-10000])->orderBy('name')->get()->each(function($rec)use($programs){
            $rec->program_name = ($prog = $programs->where('id', $rec->program)->first()) == null ? '----' : $prog->name;
        });
    ...
    public function store(Request $request){
        $validity = Validator::make($request->all(), ['name'=>'required', 'gender'=>'required', 'phone'=>'required', 'degree_id'=>'required', 'program'=>'required', 'level'=>'required']);
        if($validity->fails()){
    ...
    public function switch_confirmed(Request $request){
        ...
        $std = Students::create($student_data);
            ...
            $form_data = [
                ...
                'email'=>$resp->student->email, 'program'=>$request->program_id, 'special_needs'=>$resp->student->special_needs??null, 'father_name'=>$resp->student->father_name, 
    ...
    public function import_admit_students_save(Request $request){
        ...
        try{
            ...
            $form_data = [];
            $errors = '';
            $program = json_decode($this->api_service->programs($request->program))->data;
            $data[] = collect([
                'name'=>$request->name, 
            ...
            $form_data[$request->matric] = [
                ...
                'form' => [
                    ...
                    'email'=>'.', 'program'=>$request->program??null,
        ...
        try{
            ...
            if(($file = $request->file('file')) != null){
                ...
                while(($row = fgetcsv($reader, 1000, ',')) != null){
                    try{   
                        if(in_array($row[1], ['sex', 'SEX', 'gender', 'GENDER'])){continue;}
                        $record = [
                            'name' => $row[0],
                            'gender' => $row[1],
                            'phone' => $row[2],
