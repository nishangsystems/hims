...
@php
    $current_year = \App\Models\Batch::find(\App\Helpers\Helpers::instance()->getCurrentAccademicYear())
@endphp
@section('section')
    <div class="py-2" style="line-height: 2.3rem; font-size:larger; border-block: 2px solid gray; font-family: 'Times New Roman', Times, serif;">
        ...
        <div class="my-4 text-center font-semibold"><h4>OFFER OF ADMISSION INTO THE 16<sup>TH</sup> BATCH OF THE HND PROGRAM FOR THE ACADEMIC YEAR {{ $batch??$current_year->name??'' }}</h4></div>
        <p class="py-2 text-justify">We are delighted to inform you that the Higher Institute of Management Studies, Buea has admitted you for the <b>{{ $batch??$current_year->name??'' }} Batch</b> of its <b><span class="text-capitalize">{{ $duration??'Two' }}</span> Year Full- time {{ $degree??'' }}</b> Program in {{ $program }}, which will commence on the {{ $start_of_lectures??'' }}. Effective lectures will begin on that same date at 7:30Am.This is the beginning of an important Life-Changing Journey. You are therefore strongly advised to attend our orientation sessions when the dates are fixed so that you would be enlightened on important issues concerning the Institution, your chosen course of study and your career.</p>
